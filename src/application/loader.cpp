...
GridBase::serialized_grid_t Loader::load_grid(const std::string_view path) {
  ...
  // validate buffer, hard exit
  if (grid.buffer.size() != grid.width * grid.height * grid.stride) {
    std::cerr << "ERROR::LOADER::DESERIALIZE: Incorrect buffer size"
              << std::endl;
    std::exit(-1);
  }
...
void Loader::store_grid(const GridBase::serialized_grid_t &data,
  ...
  if (file_path.empty()) {
    ...
    std::string file_path_base = std::to_string(data.width) + "x" +
                                 std::to_string(data.height) + "x" +
                                 std::to_string(data.stride) + "_" + oss.str();

    /* append count if file already exists */
    int counter = 0;
    do {
      file_path = std::filesystem::path(
          file_path_base + (counter > 0 ? "_" + std::to_string(counter) : "") +
          ".dat");
      counter++;
  ...
  output_file.write(reinterpret_cast<const char *>(&data.width),
                    sizeof(data.width));
  output_file.write(reinterpret_cast<const char *>(&data.height),
                    sizeof(data.height));
  output_file.write(reinterpret_cast<const char *>(&data.stride),
                    sizeof(data.stride));
